...
def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
  'SELECT users.name, users.age, SUM(pledges.amount) FROM pledges INNER JOIN users ON users.id = pledges.user_id GROUP BY pledges.user_id ORDER BY users.name'
end
...
def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
  'SELECT users.name, SUM(pledges.amount) AS total FROM users INNER JOIN pledges ON users.id = pledges.user_id GROUP BY users.name ORDER BY total, users.name'
end
